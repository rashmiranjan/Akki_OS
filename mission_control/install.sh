...
SUMMARY
  # ── Convex setup (Docker mode) ────────────────────────────────────────────
  setup_convex
  return
}

setup_convex() {
  info "=== Convex Database Setup ==="
  info "Get URL + Deploy Key from: https://dashboard.convex.dev > Settings > Deploy Keys"
  local convex_url convex_deploy_key
  read -r -p "Convex Cloud URL (https://xxx.convex.cloud) [skip]: " convex_url
  if [[ -n "$convex_url" ]]; then
    convex_deploy_key="$(prompt_secret "Convex Deploy Key (dev:xxx|yyy)")"
    upsert_env_value "$REPO_ROOT/.env" "CONVEX_URL" "$convex_url"
    if [[ -n "$convex_deploy_key" ]]; then
      upsert_env_value "$REPO_ROOT/.env" "CONVEX_DEPLOY_KEY" "$convex_deploy_key"
    fi
    if command_exists npx; then
      info "Deploying Convex schema..."
      (
        cd "$REPO_ROOT/backend"
        CONVEX_DEPLOY_KEY="$convex_deploy_key" npx convex deploy
      ) && info "Convex deployed ✓" \
        || warn "Convex deploy failed — run manually: cd backend && npx convex deploy"
    else
      warn "npx not found — run manually: cd backend && npx convex deploy"
    fi
  else
    warn "Skipping Convex. Add CONVEX_URL to .env later and run: cd backend && npx convex deploy"
  fi
...
SUMMARY
  ...
  # ── Convex setup (last step, after services are running) ──────────────────
  setup_convex
}
