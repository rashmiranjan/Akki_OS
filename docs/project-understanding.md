...
- Support explicit modes:
  - install mode: can run `openclaw onboard` for first-time setup.
  - upgrade mode: must skip onboarding.
- Auto-route safety:
  - plain `install` on existing setup auto-switches to `upgrade`.
  - explicit `--mode install` on existing setup is guarded (prompt in interactive, blocked in non-interactive).
...
- Merge `mission_control/.env` using preserve-first policy:
  - existing keys are preserved (including managed keys),
...
### Mission Control `.env`
Preserve-first merged by installer. Suggested keys include:
- `OPENCLAW_TOKEN`
- `LOCAL_AUTH_TOKEN`
...
Policy:
- Existing values take precedence over generated values.
- Missing keys are appended.
- Unknown/custom keys are untouched.
